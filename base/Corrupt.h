...
INT corrupt_head(INT id, INT h, INT r, bool filter_flag = true) {
	...
	INT lef, rig, mid, ll, rr;  // 都是trainHead中triple的index
    // 找到(h,r,?)第一次出现的位置
	lef = lefHead[h] - 1;
	rig = rigHead[h];
	while (lef + 1 < rig) {
	...
	if (tmp < trainHead[ll].t) return tmp;  // random id < (h,r,?)的最小t
	if (tmp > trainHead[rr].t - (rr - ll + 1)) return tmp + rr - ll + 1;  // random id > (h,r,?)的最大t
