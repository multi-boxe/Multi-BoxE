...
class MyTrainDataLoader(object):
    ...
    def __init__(self,
                 ...
                 bern_flag=False,
                 filter_flag=True,  # 避免已知的正样本成为负样本
                 neg_ent=1,
                 neg_rel=0):
    ...
    def read(self):
        ...
        self.lib.importTrainFiles()  # 用c正常的导入训练集
        ...
        # batch_seq_size = batch_size * (1+nb_neg) -> batch_size positive, batch_size*nb_neg negative, 一个正对应nb_neg个负样本
        self.batch_seq_size = self.batch_size * (1 + self.negative_ent + self.negative_rel)

        # 创建长度为两倍batch size的numpy数组，装的不是向量表示，而是id
        # 前半部分装正样本的id，后半部分装负样本的id
        self.batch_h = np.zeros(self.batch_seq_size, dtype=np.int64)
        self.batch_t = np.zeros(self.batch_seq_size, dtype=np.int64)
        self.batch_r = np.zeros(self.batch_seq_size, dtype=np.int64)
        self.batch_hb = np.zeros(self.batch_seq_size, dtype=np.int64)
        self.batch_tb = np.zeros(self.batch_seq_size, dtype=np.int64)

        # 得到np数组的地址
        self.batch_h_addr = self.batch_h.__array_interface__["data"][0]
        self.batch_t_addr = self.batch_t.__array_interface__["data"][0]
        self.batch_r_addr = self.batch_r.__array_interface__["data"][0]
        self.batch_hb_addr = self.batch_hb.__array_interface__["data"][0]
        self.batch_tb_addr = self.batch_tb.__array_interface__["data"][0]
    ...
    def sampling(self):
        ...
        return {
            ...
            "batch_hb": self.batch_hb,
            "batch_tb": self.batch_tb,
            "mode": "normal"
        }
