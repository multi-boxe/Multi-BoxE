...
void importTrainFiles() {
	...
	for (INT i = 0; i < trainTotal; i++) {
        ...
        // 每个triple下头尾实体属于哪个box
		tmp = fscanf(fin, "%ld", &trainList[i].hb); // new
		tmp = fscanf(fin, "%ld", &trainList[i].tb); // new
	}
	fclose(fin);
    ...
    /*
    * lH rH: 头实体在trainHead排序中首次出现的位置和最后出现的位置
    * lT rT: 尾实体...
    */
	...
	// lm(rm) = 关系出现次数 / 该关系的头（尾）实体种类数?? --> 应该倒过来
...
void importTestFiles() {
    ...
    tripleList = (Triple *)calloc(tripleTotal, sizeof(Triple)); //test-train-valid，但是不需要train list
    for (INT i = 0; i < testTotal; i++) {
        tmp = fscanf(f_kb1, "%ld", &testList[i].h);
        tmp = fscanf(f_kb1, "%ld", &testList[i].t);
        tmp = fscanf(f_kb1, "%ld", &testList[i].r);
        // 读取box
        tmp = fscanf(f_kb1, "%ld", &testList[i].hb);
        tmp = fscanf(f_kb1, "%ld", &testList[i].tb);
    ...
    for (INT i = 0; i < trainTotal; i++) {
        ...
        // 读取box
        tmp = fscanf(f_kb2, "%ld", &tripleList[i + testTotal].hb);
        tmp = fscanf(f_kb2, "%ld", &tripleList[i + testTotal].tb);
    }
    ...
    std::sort(testList, testList + testTotal, Triple::cmp_rel2);    // 排序顺序: r-h-t
    ...
    // 计算每个关系在test/valid的首次出现位置和最后一次出现的位置
